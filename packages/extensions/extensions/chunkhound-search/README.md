...
When an extension registers a tool with the same name as a native Power tool, ReactorPro uses the extension's implementation instead of the built-in one. This allows you to:
...
```bash
cp -r . ~/.reactorpro/extensions/chunkhound-search
```

### 4. Configure Embedding Provider

Create a `.chunkhound.json` file in the **extension folder** (`~/.reactorpro/extensions/chunkhound-search/`):
...
```bash
ls ~/.reactorpro/extensions/chunkhound-search/.chunkhound.json
```
