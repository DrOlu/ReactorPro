...
An ReactorPro extension that automatically redacts secret values from `.env*` files when reading files.
...
1. Copy the extension directory to your ReactorPro extensions folder:

```bash
# Global (available to all projects)
cp -r redact-secrets ~/.reactorpro/extensions/
...
```bash
cd ~/.reactorpro/extensions/redact-secrets
npm install
```
