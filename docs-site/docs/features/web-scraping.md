...
ReactorPro includes a powerful web scraping feature that allows you to pull content from any URL directly into your chat context. This is incredibly useful for providing the AI with external information, such as documentation, articles, or API references.
...
When you execute the `/web` command, ReactorPro performs the following steps in the background:

1.  **Launch Internal Browser**: A hidden browser instance is launched using Electron's built-in BrowserWindow. This allows ReactorPro to render JavaScript-heavy websites, ensuring it gets the full content just as a user would see it, without requiring external dependencies.
...
4.  **Add to Context**: The cleaned text is saved to a temporary file within your project's `.reactorpro/tmp` directory. This file is then automatically added to your chat context as a **read-only** file.
