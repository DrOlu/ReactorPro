...
The `AIDER_DESK_EXTRA_PYTHON_PACKAGES` environment variable allows you to install additional Python packages into ReactorPro's virtual environment. This can be useful for a few reasons:
...
You can set this environment variable before launching ReactorPro. The value should be a comma-separated list of Python packages as you would specify them for `pip install`.
...
When you start ReactorPro, it will install the specified version of Pygments.
...
-   The packages are installed into a dedicated virtual environment managed by ReactorPro, so they will not affect your system's global Python installation.
-   If you make a mistake, you can simply unset the environment variable and restart ReactorPro. On the next start, it may try to re-install the default dependencies if needed.
-   For a clean installation, you can remove the virtual environment directory (`.reactorpro/python_venvs/reactorpro-env` inside your home directory) and let ReactorPro recreate it on the next start.
