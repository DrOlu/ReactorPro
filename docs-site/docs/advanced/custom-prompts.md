...
ReactorPro uses **prompt templates** to control how the AI agent behaves. These templates are written in [Handlebars](https://handlebarsjs.com/) format and can be customized at multiple levels to suit your specific needs.

## Available Prompt Templates

ReactorPro includes the following built-in prompt templates that you can override. You can view the default implementation of each template on GitHub to use as a starting point for your customizations:
...
| `code-change-requests.hbs` | Template for making focused code changes based on batch inline feedback in the diff viewer | [View on GitHub](https://github.com/hyperspace/reactorpro/blob/main/resources/prompts/code-change-requests.hbs) |

## Template Override System

ReactorPro supports a three-level priority system for prompt customization. When a template is requested, the system searches for it in the following order (from highest to lowest priority):

### 1. Project-Specific Prompts (Highest Priority)

**Location:** `$PROJECT_DIR/.reactorpro/prompts/`

Templates placed in your project's `.reactorpro/prompts/` directory will override all other templates for that specific project only.
...
**Location:** `~/.reactorpro/prompts/`

Templates placed in your home directory's `.reactorpro/prompts/` folder apply to all projects unless overridden by project-specific templates.
...
**Location:** Bundled with ReactorPro

These are the built-in templates that ship with ReactorPro. They are used when no custom template is found in either the project-specific or global locations. You cannot modify these directly, but you can override them by creating templates at the project or global level.
...
- **One project only** → Use `.reactorpro/prompts/` in your project
- **All projects** → Use `~/.reactorpro/prompts/`

### Step 2: Create the Prompts Directory

```bash
# For project-specific prompts
mkdir -p .reactorpro/prompts
...
**Tip:** Browse the default templates on [GitHub](https://github.com/hyperspace/reactorpro/tree/main/resources/prompts) to see how they're structured and use them as a reference for your customizations.
...
cat > .reactorpro/prompts/system-prompt.hbs << 'EOF'
...
When you create or modify a template file, ReactorPro will automatically:
...
ReactorPro provides several Handlebars helpers that you can use in your custom templates:
...
ReactorPro automatically watches for changes to custom prompt templates:
...
tail -f ~/.reactorpro/logs/reactorpro.log | grep -i prompt
```
...
2. Verify the file is in the correct directory (`.reactorpro/prompts/` or `~/.reactorpro/prompts/`)
