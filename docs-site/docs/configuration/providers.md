...
> ⚠️ **Not a Built-in Provider** — Claude Agent SDK is now available as a **community extension**. It is no longer bundled with ReactorPro.
...
1. Copy the extension to your ReactorPro extensions folder:
   ```bash
   cp -r claude-agent-sdk ~/.reactorpro/extensions/
   cd ~/.reactorpro/extensions/claude-agent-sdk
   npm install
   ```
2. Restart ReactorPro to load the extension

The extension is available in the [ReactorPro extensions repository](https://github.com/hyperspace/reactorpro/tree/main/packages/extensions/extensions/claude-agent-sdk).
...
1. Verify the extension is installed in `~/.reactorpro/extensions/claude-agent-sdk/`
2. Verify `npm install` was run in the extension directory
3. Restart ReactorPro after installation
