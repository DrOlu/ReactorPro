...
Long conversations degrade results: the context window fills up, token costs grow, and the model starts losing track of earlier details. ReactorPro provides three strategies for keeping conversations effective — plus **automatic compaction** that applies them without you noticing.

## Compaction Types

ReactorPro offers three compaction types, configurable globally in task settings and overridable per agent profile:
...
1. ReactorPro analyzes the conversation and your focus goal
...
Instead of watching token counts yourself, configure ReactorPro to compact conversations automatically:
