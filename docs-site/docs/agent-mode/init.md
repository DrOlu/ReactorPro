...
When you run `/init`, ReactorPro performs an analysis of your project's directory. It identifies key aspects like:
...
5.  **Automatic Inclusion**: Once created, `AGENTS.md` is picked up automatically — ReactorPro adds it to the agent's read-only context as a project rule, so the agent references it in every task without further setup.
...
This file is different from the `AGENTS.md` located in the root of the ReactorPro application repository, which contains guidelines for ReactorPro's own development. The one generated by `/init` is tailored *to your project* and is intended to be used by the agent during your work tasks.
...
You can find an example of the `AGENTS.md` file used for ReactorPro's own development [here](https://github.com/hyperspace/reactorpro/blob/main/AGENTS.md).
