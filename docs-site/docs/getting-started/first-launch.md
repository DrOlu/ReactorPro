...
When you start ReactorPro for the first time, an **onboarding wizard** walks you through the essential setup. You can re-run it later by resetting the onboarding flag in your settings.
...
The welcome screen introduces ReactorPro's key capabilities and lets you pick your interface language from the selector in the top-right corner (applied immediately).
...
ReactorPro does **not** block you with a long installation phase on startup. The Python environment required by the aider integration is prepared **lazily in the background**, only when it is actually needed:

1. When you send your first message that requires the Python connector, ReactorPro checks for a working `uv` installation (using your system's `uv` if present, otherwise downloading its own copy).
...
Configuration files live in `~/.reactorpro`.
