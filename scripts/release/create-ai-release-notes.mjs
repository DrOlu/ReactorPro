...
function fallbackNotes() {
  ...
  return `# ReactorPro ${releaseVersion.releaseTag}\n\nRelease ${releaseVersion.releaseTag}.`;
}
...
function normalizeMarkdown(markdown) {
  ...
  if (!output.startsWith("#")) {
    output = `# ReactorPro ${releaseVersion.releaseTag}\n\n${output}`;
  }
  return `${output.trim()}\n`;
}
...
function collectContext() {
  ...
  const repository = process.env.GITHUB_REPOSITORY?.trim() || "DrOlu/ReactorPro";
...
function buildPrompt(context) {
  return [
    ...
    "- Start with exactly this H1: # ReactorPro " + context.releaseTag,
