<p align="center">
  <img src="build/icon.png" alt="ReactorPro" width="160" />
</p>
<h1 align="center">ReactorPro</h1>
<p align="center">
  <strong>Transparent, Steerable AI Orchestration for Professional Software Engineers.</strong>
</p>

<p align="center">
  <a href="https://reactorpro.ng/docs"><img src="https://img.shields.io/badge/Docs-blue?logo=ReadTheDocs&logoColor=white" alt="Documentation" /></a>
  ...
  <a href="https://deepwiki.com/hotovo/reactorpro"><img src="https://deepwiki.com/badge.svg" alt="DeepWiki" /></a>
  <a href="https://gitmcp.io/hotovo/reactorpro"><img src="https://img.shields.io/endpoint?url=https://gitmcp.io/badge/hotovo/reactorpro" alt="GitMCP" /></a>
</p>
...
ReactorPro is an open-source agentic platform that puts **you** back in the driver's seat of AI-assisted development.
...
For additional installation options (npm, Docker, Homebrew, Scoop), see the [installation guide](https://reactorpro.ng/docs).

---

## ✨ Key Features

- 📂 **Project & Task Management:** Manage multiple codebases simultaneously without losing your train of thought. ReactorPro organizes your environment into Projects (repositories) and breaks them down into individual Tasks (features/bugs). Switch between entirely different repositories and isolated workflows instantly from a unified dashboard.
- 🌳 **Git Worktrees:** Use isolated Git worktrees to work on multiple tasks simultaneously. Each task gets its own directory where the AI can experiment, refactor, and build without ever touching or breaking your active local branch. When ready, review the diffs and merge the worktree branch back into your main flow through ReactorPro's built-in merge workflow.
...
- 🧠 **Smart Context & Memory Engine:** Powered by vector embeddings (LanceDB) and intelligent repository mapping, ReactorPro ensures the AI only loads the exact files it needs. Rely on semantic search across your codebase or explicitly pin documentation URLs and code symbols to force the AI's focus.
- 🔍 **Rich Review & Approval Gates:** Review every proposed change before it hits your disk. ReactorPro features a rich, compact diff viewer (side-by-side and unified modes) for line-by-line inspection. Configure strict tool approval gates requiring human authorization before the AI can execute shell commands or file operations.
...
ReactorPro is built to be deeply extended. We know that every enterprise and senior engineer has unique toolchains, build scripts, and workflows. The modular extension architecture allows you to inject custom logic directly into the AI's runtime and the platform's UI.

### Extensions

ReactorPro extensions go far beyond simple prompt tweaks. You can:
...
Browse [extension gallery](https://reactorpro.ng/docs/extensions/extensions-gallery) or install via CLI:
...
Bring your own data. Connect ReactorPro to any standard MCP server to give your AI assistants secure, scoped access to external databases, enterprise APIs (Jira, Linear), or internal wikis. ReactorPro can also [expose itself as an MCP server](https://reactorpro.ng/docs/features/aider-mcp-server) to other MCP-compatible clients like Claude Desktop or Cursor.

### Custom Commands & Skills

Inject your project-specific shell commands, linters, and test suites directly into the AI's toolkit, allowing it to autonomously verify its own work. Package reusable expertise into [Skills](https://reactorpro.ng/docs/features/skills) that load on-demand with progressive disclosure — keeping token usage lean while giving the AI domain-specific knowledge when needed.
...
- **REST API:** Integrate ReactorPro with external tools and workflows via a comprehensive REST API.
- **IDE Connector Plugins:** Sync context files automatically from [IntelliJ IDEA](https://plugins.jetbrains.com/plugin/26313-aiderdesk-connector) or [VS Code](https://marketplace.visualstudio.com/items?itemName=hotovo-sk.reactorpro-connector).

> For the full list of capabilities, visit our [Documentation](https://reactorpro.ng/docs).
...
[![Star History Chart](https://api.star-history.com/svg?repos=hotovo/reactorpro&type=Date)](https://star-history.com/#hotovo/reactorpro&Date)
...
ReactorPro is released under the MIT License.
