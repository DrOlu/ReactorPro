...
export function ToolResultDisplay({
          ...
          <div className="overflow-hidden rounded-lg border border-black/[0.06] bg-white/[0.55] p-2 dark:border-white/[0.08] dark:bg-white/[0.04]">
            {images.map((image, index) => (
              <ToolResultImagePreview
                // biome-ignore lint/suspicious/noArrayIndexKey: the same result may contain duplicate images (content signatures can collide on key); the list is rebuilt wholesale with the result, so the index key is stably unique.
        ...
        {details.outputMode === "count" ? null : details.outputMode === "files" ? (
          ...
          <ToolSurface className="max-h-64 overflow-auto space-y-2">
            {details.matches.map((match, index) => (
              <div
                // biome-ignore lint/suspicious/noArrayIndexKey: a multiline grep can hit the same line multiple times, producing matches with identical fields; the list is rebuilt wholesale with the result, so the index key is stably unique.
  ...
  // Browser non-screenshot result: MetaTags overview + result body (Page line / eval
  // output / a11y snapshot). Screenshot results render in the images branch above
