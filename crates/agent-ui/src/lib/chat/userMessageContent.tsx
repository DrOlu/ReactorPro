...
export type UserMessageTokenizeOptions = {
  legacyInlineFileMentions?: boolean;
};
...
function tokenizeInlineMentions(
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (marker === "@") {
      if (!options.legacyInlineFileMentions) continue;
      ...
      const reference = parseFileMentionPath(token.slice(1));
...
function tokenizeMentions(text: string, options: UserMessageTokenizeOptions): UserMessageSegment[] {
  const segments: UserMessageSegment[] = [];
  let cursor = 0;
  ...
  if (cursor < text.length) {
    appendSegments(segments, tokenizeInlineMentions(text.slice(cursor), options));
  }

  return segments.length > 0 ? segments : tokenizeInlineMentions(text, options);
}
...
export function tokenizeUserMessage(
  ...
  if (references.length === 0 || fileByPath.size === 0) {
    return tokenizeMentions(text, options);
  }
  ...
  for (const reference of references) {
    ...
    if (reference.start > cursor) {
      appendSegments(segments, tokenizeMentions(text.slice(cursor, reference.start), options));
    }
  ...
  if (cursor < text.length) {
    appendSegments(segments, tokenizeMentions(text.slice(cursor), options));
  }

  return segments.length > 0 ? segments : tokenizeMentions(text, options);
}
...
export const UserMessageContent = memo(function UserMessageContent({
  ...
  pastedTextFiles = [],
  loadCommitDetails,
  legacyInlineFileMentions = false,
}: {
  text: string;
  pastedTextFiles?: PendingUploadedFile[];
  loadCommitDetails?: CommitDetailsLoader;
  ...
  const parts = useMemo(
    () => tokenizeUserMessage(normalizedText, pastedTextFiles, { legacyInlineFileMentions }),
    [legacyInlineFileMentions, normalizedText, pastedTextFiles],
  );
