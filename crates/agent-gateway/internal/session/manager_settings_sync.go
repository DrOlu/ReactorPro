...
// settingsRemoteBool reads the boolean gate at remote.<key> from the target
// agent's settings snapshot; it is always false when the agent does not exist or
// has never synced settings (deny by default).
