...
type Config struct {
	...
	// MeshTaskExecutorWebhook is the executing edge's input-required
	// notification URL — where a peer's paused task pushes its question.
	MeshTaskExecutorWebhook string
...
func Load() *Config {
	...
	flag.StringVar(&cfg.MeshTaskExecutorWebhook, "mesh-task-executor-webhook", getenv("LIVEAGENT_GATEWAY_MESH_TASK_EXECUTOR_WEBHOOK", ""), "URL to POST a signed notification to when a task this gateway executes for a peer pauses for input (the task and its question are the payload)")
...
func (c *Config) MeshConfig() mesh.Config {
	...
	cfg.TaskExecutorWebhook = strings.TrimSpace(c.MeshTaskExecutorWebhook)
