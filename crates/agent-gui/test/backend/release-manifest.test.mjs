...
test("release updater manifest embeds generated notes and platform signatures", () => {
  ...
  try {
    writeAssetPair(dir, "ReactorPro-v9.9.9-macOS-aarch64.app.tar.gz", "sig-mac-arm");
    writeAssetPair(dir, "ReactorPro-v9.9.9-macOS-x64.app.tar.gz", "sig-mac-x64");
    writeAssetPair(dir, "ReactorPro-v9.9.9-Windows-x64-Setup.exe", "sig-win");
    writeAssetPair(dir, "ReactorPro-v9.9.9-Linux-x86_64.AppImage", "sig-linux");
    ...
    const result = spawnSync(
      ...
      [manifestScript, dir, outputPath, notesPath],
      {
        ...
        env: {
          ...process.env,
          GITHUB_REPOSITORY: "DrOlu/ReactorPro",
          RELEASE_TAG: "v9.9.9",
        },
        encoding: "utf8",
      },
    ...
    assert.equal(
      ...
      "https://github.com/DrOlu/ReactorPro/releases/download/v9.9.9/ReactorPro-v9.9.9-macOS-aarch64.app.tar.gz",
    );
...
test("release updater manifest omits generic Linux fallback without an AppImage", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "liveagent-release-"));
  try {
    writeAssetPair(dir, "ReactorPro-v9.9.9-Linux-x86_64.deb", "sig-linux-deb");
...
test("release updater manifest uses MSI for generic Windows fallback when NSIS is missing", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "liveagent-release-"));
  try {
    writeAssetPair(dir, "ReactorPro-v9.9.9-Windows-x64.msi", "sig-win-msi");
