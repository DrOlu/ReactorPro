...
test("AI release notes script calls Chat Completions API and writes markdown", async () => {
  ...
  try {
    ...
    assert.match(readFileSync(outputPath, "utf8"), /^# ReactorPro v0\.1\.6/);
  ...
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      response.end(
        JSON.stringify({
          output_text:
            "# ReactorPro v0.1.6\n\n> Release notes generated by the Responses fallback.\n\n## Overview\n\nReactorPro still publishes AI release notes when chat output is empty.",
        }),
      );
    });
  });
