...
export function createSendMessageTools(params: {
  ...
  const toolSendMessage: Tool = {
    ...
    description: [
      "Send a Markdown message through the ReactorPro Message Bus to the parent agent, all agents, or one stable delegated-agent id.",
      ...
      content: [
        {
          type: "text",
          text: [
            `Message sent to ${displayRecipientLabel(record.recipientId)} via ReactorPro Message Bus.`,
            `seq=${record.seq}`,
            `channel=${record.channel}`,
          ].join("\n"),
        },
