...
export function buildSubagentSystemPrompt(params: {
  ...
  const identityBlock = [
    `You are ${name}, a named delegated ReactorPro subagent.`,
    "",
  ...
  const lines =
    params.spec.mode === "worktree"
      ? [
          ...
          "The worktree isolates workspace file changes from the parent agent, but it does not isolate global application state. Do not modify ReactorPro settings, MCP server configuration, cron tasks, or user-level skills. MCP configuration management is not available; only enabled MCP business tools may be used.",
          ...
          params.spec.applyPolicy === "auto"
            ? "If you complete successfully, ReactorPro automatically applies your worktree patch back to the parent workspace. Your final report should describe what changed; do not tell the parent agent to manually copy your diff."
            : params.spec.applyPolicy === "explicit"
              ? "ReactorPro will apply worktree changes only when every changed file is inside the allowed output paths. Otherwise, changed files remain candidate artifacts for review."
              : "ReactorPro will not apply your worktree file changes back to the parent workspace for this task. Return the useful result in your final report.",
          params.worktree
...
export function buildMessageBusUpdateMessage(snapshot: string): Message | null {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: ["ReactorPro Message Bus snapshot refreshed for this turn.", "", text].join("\n"),
      },
    ],
    timestamp: Date.now(),
  };
