...
export const MEMORY_MANAGER_TOOL_DESCRIPTION = [
  "Manage ReactorPro's persistent local memory. Use list/read/search when you need to recall prior user/project facts, including unreviewed working memory. In the visible chat, use write/update/delete/accept when the current user asks you to remember/forget/correct something, or when the current user confirms, corrects, or clearly relies on an unreviewed memory; implicit durable preferences are handled by ReactorPro's hidden post-turn extractor.",
