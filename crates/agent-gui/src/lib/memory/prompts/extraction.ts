...
export const EXTRACTION_SYSTEM_PROMPT = [
  ...
  "",
  "You are ReactorPro's hidden post-turn memory extractor. The user never sees this exchange; only concise status text and MemoryManager tool traces may surface in developer views.",
...
export function buildExtractionInstructionPrompt(params: {
  ...
  return [
    ...
    '- User identity/role ("I am a data scientist") — explicit identity outranks git blame inferences.',
    '- Cross-project preference ("in Go projects I usually use Docker") — workspace only reflects one project; the preference is reusable.',
    '- Choice rationale ("we use X because Y") — workspace shows the choice, not the reasoning.',
    '- Workflow corrections ("run lint before tests from now on") — process rules are durable feedback even when tooling configs already encode them.',
    "",
    "Project-scope counter-examples (do NOT classify as project memory):",
    '- Merely discussing/reading project code with no workspace mutation this turn ("take a look at the logic in src/foo.ts", "what does this project use for bundling?") — gate fails. If durable, save as feedback/user/global; otherwise skip.',
    '- Project structure Q&A ("is this repo a monorepo?") — anyone can grep this; skip unless paired with a non-obvious preference.',
    "- Only read-only activity this turn (reads/searches/status checks) — gate fails even if the conversation was about the workspace.",
    '- Changes the assistant planned but did not land ("I suggest refactoring X into Y", but no actual Edit this turn) — gate fails until an actual mutation is performed.',
    "",
    "Project-scope examples that DO satisfy the gate:",
    '- <workspace-mutations-this-turn> shows an Edit on crates/foo/src/bar.rs and the user said "add #[serde(default)] to this kind of field from now on" → scope="project" type="project"; reasoning cites the Edit.',
    '- <workspace-mutations-this-turn> shows `pnpm add lodash` and the user said "this project defaults to pnpm, do not mix in npm" → scope="project"; reasoning cites the install + explicit pin.',
    '- The user explicitly says "remember that the API routes for this project live under routes/v2/" — explicit project-pin satisfies the gate without a tool call; reasoning="explicit user pin for this project".',
    "",
    "Skip with caution (these often look durable but rarely are):",
    '- "I used vim today" / "just for now" / "let me try it" — transient experiments; require an explicit signal word before promoting.',
    '- A simple factual statement ("the project uses Python 3.11") that any reader could grep in a few seconds — only save if paired with a preference or motive.',
    "",
    "Conflict policy:",
    MEMORY_CONFLICT_ARBITRATION_LINES,
