...
struct ImageProxyQuery {
    url: String,
    /// <img> requests cannot carry custom headers, so the proxy token is also
    /// accepted as a query parameter (preferred over the header for images).
    token: Option<String>,
}
...
async fn handle_image_proxy(
    ...
    // The image proxy fetches arbitrary URLs and reflects the body, so it must
    // be bound by the same proxy token as /proxy/{provider} — either as the
    // standard header or, because <img> tags cannot set headers, as a
    // ?token= query parameter generated by prepareImageProxyUrl().
    ...
    if !token_matches(&provided_token, &state.info.token) {
    ...
    .await
    {
        return error_response(StatusCode::BAD_REQUEST, &message, &headers);
    }
...
/// Length-independent constant-time equality over SHA-256 digests (same
/// approach as the gateway's auth middleware) so token checks do not leak
/// prefix-match timing.
...
/// Hosts the image proxy must never dial: loopback, RFC1918/CGNAT, link-local
/// (cloud metadata), benchmark, multicast/reserved, and IPv6 equivalents.
...
/// Hostname forms that must never be dialed even before resolution.
...
/// Resolve `host` and reject when it is a literal or resolves to a blocked
/// target. Fails closed on resolution errors.
async fn ensure_publicly_dialable(host: &str, port: u16) -> Result<(), String> {
    if is_blocked_host_literal(host) {
        return Err(format!("Image URL host '{host}' is not allowed"));
    }
    if let Ok(ip) = host.parse::<std::net::IpAddr>() {
        return if is_blocked_ip(ip) {
            Err(format!("Image URL host '{host}' is not allowed"))
        } else {
            Ok(())
        };
    }
    ...
    for addr in addrs {
        if is_blocked_ip(addr.ip()) {
            return Err(format!("Image URL host '{host}' resolves to a disallowed address"));
        }
    }
    Ok(())
}
...
async fn handle_proxy(
    ...
    match required_header(&headers, PROXY_TOKEN_HEADER) {
        Ok(value) if token_matches(&value, &state.info.token) => {}
