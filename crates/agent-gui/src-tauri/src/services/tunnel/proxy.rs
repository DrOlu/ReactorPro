...
async fn run_tunnel_http_request(
    ...
    let result = async {
        ...
        // Tunnel targets are always local/intranet services: explicitly ignore environment
        // proxies so an OS-level HTTP(S)_PROXY cannot hijack local forwarding and break the tunnel.
