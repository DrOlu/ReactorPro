...
/// Relative path recorded in checkpoints: must be relative to the real target
/// **after** canonicalize.
///
/// `resolve_target` resolves symlinks inside the workspace, so the requested
/// path (`link/a.txt`) and the actual on-disk path (`real/a.txt`) may diverge.
/// Recording by requested path has two consequences: the pre-image hangs on a
/// path that was never modified; and during rollback, rejecting symlinks level
/// by level would classify it as unresolvable, so this change could never be
/// rolled back. When no prefix can be obtained (which theoretically should not
/// happen), fall back to the requested path.
...
/// Create the target's parent directory and return its canonical form. The
/// return value is not a nice-to-have: a newly created file has no canonicalize
/// entry point, so if the parent directory goes through a symlink inside the
/// workspace (`link/new.txt`), the unresolved raw path would be recorded as the
/// on-disk target in the checkpoint, and since the rollback side rejects
/// symlinks level by level, that record could never be rolled back. Callers
/// must join the target with the returned real parent directory.
...
fn decode_xml_entities(input: &str) -> String {
    // Office document XML is generated by software and normally decodes fine; on
    // an invalid entity, degrade by keeping the original text.
...
fn read_url_image_source(source: &str) -> Result<ReadResponse, FsError> {
    ...
    // Image.url lets the model read remote images: the same semantics as the
    // chat image reverse proxy -- go out through the proxy when the app proxy
    // is enabled, and fail fast on a bad proxy config instead of silently
    // connecting directly.
...
fn fs_write_text_impl(
    ...
    let (target, existed_before) = match fs::symlink_metadata(&raw_target) {
        ...
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            // Join the target with the canonical parent directory: the requested
            // path may go through a symlink inside the workspace (`link/new.txt`),
            // and joining the raw path would make the checkpoint record the link
            // path, which the rollback side rejects level by level, so this new
            // file could never be deleted.
    ...
    // Capture the pre-image before writing to disk (root + relative path): if
    // absent, record a deletion marker; if present, copy the original bytes.
    // Failure does not block the write.
    capture_pre_image(
        checkpoint.as_ref(),
        &path.root,
...
fn fs_edit_text_impl(
    ...
    // Capture the pre-image before writing to disk: reuse the original bytes
    // already read into memory above. Failure does not block the write.
    capture_pre_image(
        checkpoint.as_ref(),
        wd,
...
fn fs_delete_impl(
    ...
    let kind = if meta.file_type().is_symlink() {
        // Do not capture a pre-image for symlinks: the link target is not this
        // file's content and the restore semantics are unclear.
        remove_symlink_path(&target)?;
        "symlink"
    } else if meta.is_file() {
        // Capture the entire file content before deletion so rollback can restore
        // it exactly. Failure does not block the deletion.
        capture_pre_image(checkpoint.as_ref(), wd, &ckpt_rel, PreImage::File(None));
        fs::remove_file(&target)?;
        "file"
    } else if meta.is_dir() {
        // A directory is deleted recursively, so only an unrecoverable marker can be
        // recorded, accurately reflected by the diff statistics.
...
pub(crate) fn spawn_workspace_open_command(target: &Path, mode: &str) -> Result<(), String> {
    // We do not wait for the launcher process, but it must be reaped: if the
    // Child is dropped directly, no one wait()s after the subprocess exits, and
    // every open/reveal in Finder/Explorer leaves a <defunct> under this
    // process (the zombie accumulation in long sessions reported in the field
    // traces back to this path).
...
fn workspace_open_command(target: &Path, mode: &str) -> Command {
    ...
    if mode == "reveal" {
        // explorer.exe parses the command line itself: an unquoted path is
        // truncated at the first space (for example
        // `D:\Videos\JianyingPro Materials\a.txt` becomes
        // `D:\Videos\JianyingPro`, and when the path is invalid explorer falls
        // back to opening the default "Documents" directory). Yet the Rust
        // standard library's default quoting wraps all of `/select,<path>` in
        // quotes, which explorer also cannot parse. Use raw_arg here to bypass
        // the default quoting and quote only the path portion:
        // `/select,"<path>"`.
...
mod tests {
    ...
    // When the path contains spaces, quoting must preserve the whole path;
    // otherwise explorer truncates the argument at the space and falls back to
    // opening the default "Documents" directory (the file-tree "open containing
    // folder" space-in-path bug).
    #[cfg(target_os = "windows")]
    #[test]
    ...
    fn decode_xml_entities_covers_named_and_numeric_forms() {
        ...
        // Invalid entities degrade gracefully: keep the original text.
        assert_eq!(
    ...
    fn write_new_file_through_internal_symlink_resolves_real_parent() {
        ...
        // Create a file through a symlink inside the workspace: the on-disk target
        // must be the resolved real path, otherwise the checkpoint records the
        // link path, and the rollback side rejects symlinks level by level, so
        ...
        // checkpoint_rel must receive the real relative path real/new.txt.
