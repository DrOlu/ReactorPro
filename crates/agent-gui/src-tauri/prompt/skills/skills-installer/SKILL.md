...
description: Install, list enabled, validate, or package ReactorPro skills. Use when you need to inspect the skills enabled in the current conversation, import a local skill directory or package, search/install from ClawHub, install from a GitHub repo/tree URL, or reconcile conflicts during an upgrade.
---

# Skills Installer

Install ReactorPro skills into the fixed runtime root while preserving existing user data.
...
2. Determine the target destination. Runtime installs always go through `SkillsManager` into ReactorPro's fixed user Skills root.
3. For local workspace sources, pass workspace-relative paths such as `./my-skill` or `./dist/my-skill.skill`; ReactorPro resolves them against the current chat workspace before installing into the fixed skills root.
...
8. After installation, the new skill is enabled for the current conversation automatically; ask ReactorPro to rescan skills or reopen the Skills menu only if the new skill does not appear immediately.
