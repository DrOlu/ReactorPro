...
Use this reference before installing or replacing ReactorPro skills.
...
- Parse `name` and `description` before installing. For `README.md` without frontmatter, ReactorPro derives the skill name from the directory and loads the full README when enabled.
...
- Do not manually edit ReactorPro Settings. SkillsManager automatically enables newly installed skills for the current conversation when installation succeeds.

## After Install

ReactorPro discovers skills through a scan of the fixed skills root. After an install, ReactorPro refreshes discovery; if the UI still has stale state:
