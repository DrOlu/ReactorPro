# ReactorPro Skill Format

Use this reference when writing or reviewing a ReactorPro-compatible skill.
...
- ReactorPro discovers runtime skills from the fixed user Skills root managed by `SkillsManager`.
...
- Markdown skill entries should begin with YAML frontmatter containing `name` and `description`. `README.md` without frontmatter is supported only as an import/discovery fallback; ReactorPro derives the name from the directory and loads the full README when enabled.
...
---
name: my-skill
description: Do a specific ReactorPro workflow. Use when the user asks for the exact tasks this skill supports.
---
```
...
`allowed-tools` is accepted only as compatibility metadata when importing existing skills. ReactorPro currently enforces tool access through the chat Skills selector and runtime `SkillAccessPolicy`, not through per-skill frontmatter.
...
- Use `SkillsManager` actions for ReactorPro skill creation, installation, listing, validation, and packaging. Do not bundle Python helper scripts for these core workflows.
