---
name: skills-creator
description: Create or update ReactorPro runtime skills. Use when you need to summarize a workflow into a skill, write it into the fixed user skills root, validate it, package it, or refine a skill's references for ReactorPro's SkillsManager flow.
---

# Skills Creator

Create ReactorPro-compatible skills in the fixed runtime skills root by keeping the entrypoint concise, moving long guidance into references, and using helper files only when the workflow truly needs them. Prefer English for generated skill documentation so skills stay broadly reusable, but other languages are accepted.
...
8. After creation, the new skill is enabled for the current conversation automatically; ask ReactorPro to rescan skills or reopen the Skills menu only if the new skill does not appear immediately.
